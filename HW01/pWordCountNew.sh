...
#              counts to './$reducer' -- don't forget to pipe the output
...
for datachunk in $data.chunk.*; do
    # pipe the lines of text into the user supplied executable (mapper)
    # and redirect STDOUT to a temporary file on disk
	#./$mapper  < $datachunk > $datachunk.counts &
    python $mapper  < $datachunk > $datachunk.counts &
done
